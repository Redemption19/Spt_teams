...
import { InvoiceTemplateService, InvoiceTemplateFormData } from '@/lib/invoice-template-service';

const initialFormData: InvoiceTemplateFormData = {
  name: '',
  description: '',
  category: 'business',
  isDefault: false,
...
export default function CreateTemplatePage() {
  ...
  const [formData, setFormData] = useState<InvoiceTemplateFormData>(initialFormData);
  const [loading, setLoading] = useState(false);
  const [activeSection, setActiveSection] = useState('basic');

  const handleInputChange = (field: keyof InvoiceTemplateFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const templateId = await InvoiceTemplateService.createTemplate({
        ...formData,
  ...
  const handlePreview = () => {
    // Create a temporary preview with current form data
    const previewData = {
      ...formData,
      id: 'preview-temp',
      workspaceId: currentWorkspace?.id || '',
    ...
    sessionStorage.setItem('template-preview-data', JSON.stringify(previewData));
    
    // Open preview in a new window
    window.open('/dashboard/financial/invoices/templates/preview/temp', '_blank');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Create Invoice Template</h1>
          <p className="text-muted-foreground">
            Design a custom template for your invoices
          </p>
        </div>
      </div>
