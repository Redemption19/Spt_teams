...
import { AttendanceService, AttendanceRecord, UpdateAttendanceData } from '@/lib/attendance-service';
...
export default function EditAttendanceRecordPage() {
  ...
  }, [shouldShowCrossWorkspace, user?.uid]);
  ...
  const loadRecord = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const attendanceRecord = await AttendanceService.getAttendanceRecord(recordId);
      setRecord(attendanceRecord);
      setSelectedWorkspace(attendanceRecord.workspaceId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepare update data, filtering out empty strings and undefined values
      const updateData: UpdateAttendanceData = {
        updatedBy: user?.uid || ''
      };

      // Only add fields if they have values or have changed
      if (formData.clockIn && formData.clockIn.trim()) {
        updateData.clockIn = formData.clockIn.trim();
      }
      if (formData.clockOut && formData.clockOut.trim()) {
        updateData.clockOut = formData.clockOut.trim();
      }
      if (formData.breakStart && formData.breakStart.trim()) {
        updateData.breakStart = formData.breakStart.trim();
      }
      if (formData.breakEnd && formData.breakEnd.trim()) {
        updateData.breakEnd = formData.breakEnd.trim();
      }
      if (formData.status !== record.status) {
        updateData.status = formData.status;
      }
      if (formData.location.trim() !== (record.location || '')) {
        updateData.location = formData.location.trim();
      }
      if (formData.notes.trim() !== (record.notes || '')) {
        updateData.notes = formData.notes.trim();
      }

      await AttendanceService.updateAttendance(record.id, updateData);
      ...
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Modify an existing attendance record</p>
          </div>
        </div>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Record Not Found</h1>
            <p className="text-muted-foreground mt-1">The attendance record you're looking for doesn't exist</p>
          </div>
        </div>
        ...
        <Card className="card-enhanced">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">Attendance Record Not Found</h3>
            <p className="text-muted-foreground mb-4">
              The attendance record you're trying to edit doesn't exist or you don't have permission to access it.
            </p>
            <Link href="/dashboard/hr/attendance">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-2 p-4 rounded-lg bg-muted/30 border border-border/30">
        ...
        <span className="text-sm text-muted-foreground">Editing Record:</span>
        <span className="text-sm font-medium text-foreground">{record.employeeName}</span>
        <span className="text-sm text-muted-foreground">•</span>
        <span className="text-sm text-muted-foreground">{format(parseISO(record.date), 'MMM dd, yyyy')}</span>
        <span className="text-sm text-muted-foreground">•</span>
      ...
      <Card className="card-enhanced">
        <CardHeader>
          ...
          <CardDescription className="text-muted-foreground">
            Modify the attendance information for this record
          </CardDescription>
        </CardHeader>
        <CardContent>
